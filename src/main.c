...
static void bl_export_if_needed(int chip, int pwm) {
    char pwm_path[128];
    snprintf(pwm_path, sizeof(pwm_path), "/sys/class/pwm/pwmchip%d/pwm%d", chip, pwm);
...
static void bl_config(int chip, int pwm, int period_ns, int duty_ns, int enable) {
    bl_export_if_needed(chip, pwm);
    char path[160];
    snprintf(path, sizeof(path), "/sys/class/pwm/pwmchip%d/pwm%d/enable", chip, pwm);
    bl_write(path, "0");
    snprintf(path, sizeof(path), "/sys/class/pwm/pwmchip%d/pwm%d/period", chip, pwm);
    char buf[32]; snprintf(buf, sizeof(buf), "%d", period_ns); bl_write(path, buf);
    snprintf(path, sizeof(path), "/sys/class/pwm/pwmchip%d/pwm%d/duty_cycle", chip, pwm);
    snprintf(buf, sizeof(buf), "%d", duty_ns); bl_write(path, buf);
    snprintf(path, sizeof(path), "/sys/class/pwm/pwmchip%d/pwm%d/enable", chip, pwm);
...
static lv_color_t buf0[160 * 16];
static lv_color_t buf1[160 * 16];

static void show_blink_eye(lv_disp_t *disp, const char *eye_path, const char *lid_path)
{
...
int main(int argc, char **argv)
{
    ...
    lv_display_set_buffers(disp0, buf0, buf1, sizeof(buf0), LV_DISPLAY_RENDER_MODE_PARTIAL);
    ...
    lv_display_set_buffers(disp1, buf0, buf1, sizeof(buf0), LV_DISPLAY_RENDER_MODE_PARTIAL);
